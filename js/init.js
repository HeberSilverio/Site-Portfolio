...
function arlo_tm_palestras(){
	...
	if(jQuery().isotope) {
		...
		// Needed variables
		var list 		 = jQuery('.arlo_tm_palestras_list');
		var filter		 = jQuery('.arlo_tm_palestras_filter');
...
function arlo_tm_projects() {
	...
	jQuery('.arlo_tm_palestras_animation_wrap').each(function() {
		jQuery(this).on('mouseenter', function() {
			if (jQuery(this).data('title')) {
				jQuery('.arlo_tm_palestras_titles').html(jQuery(this).data('title') + '<span class="work__cat">' + jQuery(this).data('category') + '</span>');
				jQuery('.arlo_tm_palestras_titles').addClass('visible');
			}

			jQuery(document).on('mousemove', function(e) {
				jQuery('.arlo_tm_palestras_titles').css({
					left: e.clientX - 10,
					top: e.clientY + 25
				});
			});
		}).on('mouseleave', function() {
			jQuery('.arlo_tm_palestras_titles').removeClass('visible');
		});
	});
}
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["MVP por 17 anos consecutivos", "Especialista em Windows", "Especialista em Segurança Digital", "Consultor de alto nível", "Palestrante"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
